# Claude Code Project Instructions — ratatui-macros-kotlin

## Project Overview

This is **ratatui-macros-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`ratatui-macros`](https://crates.io/crates/ratatui-macros).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/ratatui-macros/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/ratatui-macros>
